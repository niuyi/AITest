...
class BPNet(object):
	...
	def init_hiddenWB(self):
		self.hi_w = 2.0*(random.rand(self.nHidden,self.nSampDim)-0.5) # 4*2
		self.hi_b = 2.0*(random.rand(self.nHidden,1)-0.5) # 4*2
		self.hi_wb = mat(self.addcol(mat(self.hi_w),mat(self.hi_b)))
        #构造一个矩阵，行数等于隐藏单元的个数，前N列（N为输入维度）为W，最后一列是b
        #也就是说每个隐藏单元，对应n个w，一个b
	# 输出层初始化
	def init_OutputWB(self):
        ...
        # 也就是说每个输出单元，对应x（x为隐藏单元的个数）个w，一个b

	# bp网主程序
	def bpTrain(self):
		# 数据集矩阵化
		# dataMat 307*3，包括标签列
		SampIn = self.dataMat.T  # 3 * 307
		expected = mat(self.classLabels)

		print('sampin m n', shape(SampIn))
		print('expected m n', shape(self.classLabels))
...
# 数据集
bpnet = BPNet()
bpnet.loadDataSet("data/testSet2.txt")
bpnet.dataMat = bpnet.normalize(bpnet.dataMat)

(m, n) = shape(bpnet.dataMat)
print(m,n) # 307 * 3
#
# # 绘制数据集散点图
