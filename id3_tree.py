...
class ID3DTree(object):
    ...
    def loadDataSet(self, path, labels):
        ...
        recordlist = [row.decode().split("\t") for row in rowlist if row.strip()]
...
# dtree = ID3DTree()
# dtree.loadDataSet("data/id3-dataset.dat",["age","revenue","student","credit"])
# dtree.train()
#
...
test = [1, 2, 3, 1, 1, 2, 5, 6, 1, 3]

items = dict([(test.count(x), x) for x in test ])
print(items[max(items.keys())])
#
# print(dict([(1,3), (2,4), (1,5)]))